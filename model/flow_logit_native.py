"""exp 012 — Riemannian baseline with model living in *logit* chart space.

Symmetric chart: y = (logit(cx), logit(cy), logit(w), logit(h)).
All four components get the same multiplicative transform — unlike `flow_chart_native`
which leaves position as identity.

- model state = logit chart y ∈ R^4
- trajectory  = chart-space straight line: y_t = (1-t) y_0 + t y_1
- u_target    = constant y_1 - y_0
- ODE Euler integrates in logit space; decode with sigmoid at the end.

Init b_0 prior is shared with SignalFlowModel (configurable via init_prior).
"""
...
from .trajectory import logit_decode, logit_encode, sample_init_box


class LogitNativeFlowModel(nn.Module):
    """Riemannian baseline that lives entirely in logit chart state."""
    ...
    def forward(
        self,
        y_t: torch.Tensor,    # (B, n_q, 4) logit chart state
        t: torch.Tensor,      # (B,)
    ...
    def fm_loss(
        ...
        y_0 = logit_encode(b_0)
        y_1 = logit_encode(b_1)
        B = b_1.shape[0]
    ...
    def sample(
        ...
        """ODE Euler in logit chart space. Returns (boxes, traj_boxes[K+1]) in box space."""
        device = image.device
        B = image.shape[0]
        if init_box is None:
            ref = torch.empty(B, n_queries, 4, device=device)
            init_box = sample_init_box(ref, prior=self.init_prior)
        y = logit_encode(init_box.to(device))
        traj = [y.clone()]
        dt = 1.0 / K
        for k in range(K):
            ...
            y = y + dt * v
            traj.append(y.clone())
        boxes = logit_decode(y).clamp(0, 1)
        traj_boxes = [logit_decode(z).clamp(0, 1) for z in traj]
        return boxes, traj_boxes


if __name__ == "__main__":
    torch.manual_seed(0)
    model = LogitNativeFlowModel(
    ...
    # 1) loss finite + u_target constant in t (Riemannian core)
    ...
    # 3) sample → boxes ∈ [0,1] guaranteed (sigmoid)
    K = 8
    ...
    print("flow_logit_native sanity check 통과")
